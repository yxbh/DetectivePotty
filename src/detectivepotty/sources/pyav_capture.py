...
codebase relies on (``read`` / ``get`` / ``isOpened`` / ``release``) so it drops into
the existing ``capture_factory`` seams (``FileSource``, ``harvest``,
``dataset_export``, ``experiment.groundtruth``) with no call-site changes. Random
frame seeking (the tuner / preview surfaces) is intentionally out of scope — those
keep ``cv2``.
...
class PyAvCapture:
    """A ``cv2.VideoCapture``-compatible sequential reader backed by PyAV.

    Only sequential ``read()`` is supported (no random seeking). Frames are decoded
    in container order and returned as contiguous BGR ``uint8`` ndarrays, matching
    OpenCV's output so the surrounding frame-index/timestamp math is unchanged.
    """
